export interface EdgeScreenDevice {
  screenId: string;
  screenName: string;
  applianceId?: string;
  ipAddress?: string;
...
function makeScreens(count: number, enabledCount: number, theatreSeed: number): EdgeScreenDevice[] {
  return Array.from({ length: count }, (_, i) => {
    const hasDevice = i < enabledCount;
    const status: EdgeScreenDevice["status"] = hasDevice
    ...
    return {
      ...
      hasDevice,
      applianceId: hasDevice ? `EDGE-${1000 + theatreSeed * 100 + i}` : undefined,
...
export const edgeTheatres: EdgeTheatre[] = Array.from({ length: 60 }, (_, i) => {
...
export interface EdgeLookupTheatre {
  id: string;
  theatreId: string;
  theatreName: string;
  city: string;
...
export const edgeLookupTheatres: EdgeLookupTheatre[] = Array.from({ length: 80 }, (_, i) => {
