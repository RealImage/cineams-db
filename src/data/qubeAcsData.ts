export interface QubeAcsScreenDevice {
  screenId: string;
  screenName: string;
  applianceId?: string;
  ipAddress?: string;
...
export interface QubeAcsTheatre {
  id: string;
  theatreId: string;
  theatreName: string;
  alsoKnownAs?: string;
...
function makeScreens(count: number, enabledCount: number, theatreSeed: number): QubeAcsScreenDevice[] {
  return Array.from({ length: count }, (_, i) => {
    const hasDevice = i < enabledCount;
    const status: QubeAcsScreenDevice["status"] = hasDevice
    ...
    return {
      ...
      hasDevice,
      applianceId: hasDevice ? `QACS-${1000 + theatreSeed * 100 + i}` : undefined,
...
export const qubeAcsTheatres: QubeAcsTheatre[] = Array.from({ length: 60 }, (_, i) => {
  ...
  return {
    ...
    networkId: `NET-${1000 + i}`,
    networkPassword: "********",
    screens: makeScreens(total, enabled, i + 1),
  };
});

// Lookup pool for Add Theatre search (theatres NOT yet enabled)
export interface LookupTheatre {
  id: string;
  theatreId: string;
  theatreName: string;
  city: string;
...
export const lookupTheatres: LookupTheatre[] = Array.from({ length: 80 }, (_, i) => {
  const loc = rand(cities, i + 3);
  const chain = rand(chains, i + 1);
  const tName = rand(theatreNames, i + 5);
  return {
    id: `lk-th-${i + 1}`,
    theatreId: `T${String(20000 + i)}`,
