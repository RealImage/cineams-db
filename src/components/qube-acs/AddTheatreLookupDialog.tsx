...
import { lookupTheatres, qubeAcsTheatres } from "@/data/qubeAcsData";
...
export const AddTheatreLookupDialog = ({ open, onOpenChange }: Props) => {
  const navigate = useNavigate();
  const [query, setQuery] = useState("");
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const enabledIds = useMemo(() => new Set(qubeAcsTheatres.map((t) => t.theatreId)), []);

  const matches = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return [];
    return lookupTheatres
  ...
  const proceed = () => {
    ...
    onOpenChange(false);
    navigate(`/qube-appliances/qube-acs/add/${selectedId}`);
  };
