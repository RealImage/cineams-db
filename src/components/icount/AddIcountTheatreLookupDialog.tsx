...
import { icountLookupTheatres, icountTheatres } from "@/data/icountData";
...
export const AddIcountTheatreLookupDialog = ({ open, onOpenChange }: Props) => {
  ...
  const enabledIds = useMemo(() => new Set(icountTheatres.map((t) => t.theatreId)), []);

  const matches = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return [];
    return icountLookupTheatres
