...
interface EditSuiteDialogProps {
  ...
  onSave: (suiteData: Partial<ScreenSuite>) => void;
}

export const EditSuiteDialog = ({ open, onOpenChange, suite, availableDevices, onSave }: EditSuiteDialogProps) => {
  ...
  useEffect(() => {
    if (suite) {
      setEffectiveFromDate(suite.effectiveFromDate);
      setSelectedDeviceIds(suite.devices.map(device => device.id));
    }
  }, [suite]);

  // Combine currently assigned devices with available devices, removing duplicates
  const allDevices = suite 
    ? [...suite.devices, ...availableDevices.filter(device => 
        !suite.devices.some(assignedDevice => assignedDevice.id === device.id)
      )]
    : availableDevices;
    
  // Filter devices to only show those with valid certificates
  const validDevices = allDevices.filter(device => device.certificateStatus === "Active");
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update suite information and manage device assignments.
          </DialogDescription>
        </DialogHeader>
