  ...
  return <motion.aside className={`fixed inset-y-0 left-0 z-50 bg-background border-r border-border py-2 flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16 px-2' : 'w-64 px-3'}`} initial={isMobile ? {
    ...
    x: 0
      ...
      <nav className="space-y-1 flex-1 overflow-auto">
        ...
        {/* Theatre TDL with submenu */}
        <NavItemWithSubmenu icon={Monitor} label="Theatre TDL" basePath="/theatre-device-management" subItems={theatreDeviceManagementSubItems} isCollapsed={isCollapsed} />
