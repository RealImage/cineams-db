...
export const emptyPulseFilters: PulseFilters = { chain: "all", location: "all" };
...
export const PulseFilterPanel = ({ open, onOpenChange, chains, locations, filters, onChange, onClear }: Props) => {
  ...
  const set = (patch: Partial<PulseFilters>) => setDraft((d) => ({ ...d, ...patch }));
  return (
    <FilterDrawer
      open={open}
      onOpenChange={onOpenChange}
      onApply={() => onChange(draft)}
      onClear={() => { setDraft(emptyPulseFilters); onClear(); }}
    >
