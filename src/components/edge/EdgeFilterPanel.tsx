...
export const emptyEdgeFilters: EdgeFilters = { chain: "all", location: "all" };
...
interface Props {
  ...
  onChange: (next: EdgeFilters) => void;
  onClear: () => void;
}

export const EdgeFilterPanel = ({ open, onOpenChange, chains, locations, filters, onChange, onClear }: Props) => {
  const [draft, setDraft] = useFilterDraft(filters, open);
  const set = (patch: Partial<EdgeFilters>) => setDraft((d) => ({ ...d, ...patch }));
  return (
    <FilterDrawer
      open={open}
      onOpenChange={onOpenChange}
      onApply={() => onChange(draft)}
      onClear={() => { setDraft(emptyEdgeFilters); onClear(); }}
    >
