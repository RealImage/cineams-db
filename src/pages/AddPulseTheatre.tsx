...
import type { PulseScreenDevice } from "@/data/pulseData";
...
const AddPulseTheatreForm = ({ theatre }: { theatre: ApplianceLookupDetail }) => {
  const navigate = useNavigate();
  const save = useEnrolApplianceTheatre("pulse");
  ...
  const [screens, setScreens] = useState<PulseScreenDevice[]>(() =>
  ...
  const updateScreen = (id: string, patch: Partial<PulseScreenDevice>) => {
  ...
  const onSave = () => {
    save.mutate({ theatreId: theatre.id, ...details, screens }, {
      onSuccess: () => {
        toast.success(`${theatre.theatreName} added to Pulse`);
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold">Add Theatre to Pulse</h2>
              ...
              <div className="space-y-6">
                ...
                <section className="space-y-3">
                  ...
                  {!active.hasDevice && !editDevice ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1"><Label className="text-xs">Pulse Appliance ID</Label><Input value={active.applianceId ?? ""} onChange={(e) => updateScreen(active.screenId, { applianceId: e.target.value })} placeholder="PULSE-XXXXX" /></div>
...
const AddPulseTheatre = () => {
  const { lookupId } = useParams();
  const theatreQuery = useApplianceLookupTheatre("pulse", lookupId);
  return (
    <QueryState query={theatreQuery} label="theatre">
      {(theatre) => <AddPulseTheatreForm key={theatre.id} theatre={theatre} />}
    </QueryState>
  );
};

export default AddPulseTheatre;
